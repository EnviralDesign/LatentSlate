...
- **2026-05-21:** Hardened the shared egui vertical scroll helper so inspector/provider/modal scroll regions allocate a fixed clipped viewport, cap horizontal width, disable scroll-edge fades in editor panes, and clamp inner content width. Attribute inspector cards now also clamp their content to the rail width so long generative fields truncate/scroll vertically instead of painting into the preview/timeline area.
