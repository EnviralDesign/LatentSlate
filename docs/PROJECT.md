...
- The current generic executor dispatches ComfyUI, OpenAI image, xAI image, and xAI Grok video providers through the same generation queue/output-version path.
...
3.  **Display (egui/eframe)**
    ...
    - The preview canvas supports auto-fit and manual view control. Manual scroll zoom and right-drag pan disable auto-fit, while selected visual clips expose the same transform state through on-canvas move/scale/rotate handles and the Attributes inspector.
...
- [x] **Provider System**
  ...
  - [x] OpenAI image, xAI image, and xAI Grok video cloud adapters using app-managed credential IDs
...
- **2026-05-29:** Updated timeline/inspector provider choice menus to use compact accent rows with the same output-type color language as Provider Builder provider rows, plus the provider workflow short label for context.
- **2026-05-28:** Centralized FFmpeg initialization and set the app's default native FFmpeg log level to `Error`, suppressing warning-level H.264 seek noise such as `mmco: unref short failure` during scrub-heavy preview work while preserving real decoder/open errors. Developers can temporarily restore FFmpeg warning/debug output with `NLA_FFMPEG_LOG=warning`, `debug`, or another supported FFmpeg log level.
- **2026-05-28:** Froze timeline hover and pointer handling while context menus/popups are open. Timeline context menus now keep the original right-click anchor position, so moving the mouse across menu items no longer changes hover labels, target-track highlights, scrollbars, or selected clips/tracks underneath the menu.
- **2026-05-28:** Reorganized the timeline context menu generation actions so video-track text-to-image creation now appears inside the existing `Generate` section as `Create T2I Image`, alongside the selected-clip `I2I` and `I2V` actions, instead of as a separate `New Generative Image` item above `Open Asset Lab`.
- **2026-05-28:** Began the egui code organization refactor toward soft file-size targets. App shell chrome, Timeline panel orchestration/interaction plus timeline geometry/painting, the Attributes inspector/sidebar orchestration, the automation UI harness glue, the Generation Queue popover/orchestration, Assets pane orchestration/row/render/sort helpers, Preview canvas rendering/interaction/transform helpers, Preview/runtime scheduling and thumbnail cache helpers, Asset Lab state/render/media/modal orchestration helpers, Export modal state/settings/UI/event helpers, Provider Builder state/UI helpers, Provider modal/API key/JSON editor orchestration, generation runtime queue/event helpers, destructive-action confirmation modals, and project/setup modal helpers now live under `src/egui_app/`, keeping the existing UI behavior path intact while reducing the monolithic `egui_app.rs` surface and establishing a low-risk child-module extraction pattern for future UI feature splits.
- **2026-05-28:** Fixed Provider Builder schema enrichment for ComfyUI `COMBO` nodes whose live `/object_info` schema reports an empty options list while the workflow stores choices in node-local `option*` inputs. Refreshing schema now backfills enum options from those workflow values; the local ignored `reg-klein4b9b-t2i-API` manifest was also patched for immediate testing.
- **2026-05-27:** Added provider generation-intent metadata and timeline generation menus. Providers now carry a `workflow_kind` (`auto`, `text_to_image`, `image_to_video`, `first_frame_last_frame_video`, etc.), the ComfyUI Provider Builder exposes a Generation selector, provider rows show the resolved short kind, and timeline/inspector generation actions now filter provider choices by intent. Right-clicking a video track can create a new generative image clip plus asset at the clicked time, selected image clips can spawn I2I or I2V clips with a chosen provider, and Generate Between Keyframes can target compatible first/last-frame video providers instead of relying on a hardcoded provider assumption.
- **2026-05-27:** Made Provider Builder workflow selection auto-name Comfy providers. Picking a workflow JSON now immediately populates the provider Name field from the workflow file stem so new providers start with a useful name without manual copy/paste.
- **2026-05-27:** Refined Asset Lab video scrubber layout. The active/total timecode now lives in the Preview header metadata area instead of inside the scrub strip, preventing overlap with ticks/playhead, and the preview-to-scrubber spacing uses a tighter local gap.
- **2026-05-27:** Fixed preview right-drag panning while transform handles are visible. Selected clip gizmo interactions now only start move/scale/rotate drags from the primary mouse button, so secondary-button viewport panning is no longer overwritten by the transform overlay.
- **2026-05-27:** Brought image assets closer to Asset Lab parity. Normal image assets now use the same high-resolution lab preview path with pan/zoom/reset-to-fit instead of the small thumbnail, share the fixed-preview plus scrolling-details layout, expose dimensions/file metadata, and provide lab actions for duplicate, add to timeline, open location, and delete confirmation. The same media-path resolver also lets normal videos use the Asset Lab scrubber/preview path where available.
- **2026-05-27:** Added vertical clip drag retargeting in the timeline. Dragging a clip body now keeps the existing horizontal move/snap behavior while also moving the clip onto the compatible video or audio track row under the pointer, with the hovered target row highlighted during the drag.
- **2026-05-27:** Fixed View menu panel toggles to use positive visibility semantics. Assets, Attributes, and Timeline are now checked when visible and unchecked when collapsed, matching the Preview Stats opt-in checkbox behavior.
- **2026-05-27:** Polished Asset Lab preview behavior. Open clip actions now seed Asset Lab to the clip-local playhead time, modals block background preview/timeline scroll input, selected generative video versions decode directly in the lab preview without resetting the local scrubber, the preview supports local pan/zoom with reset-to-fit, video versions get a frame-accurate local scrubber with second ticks and arrow-key frame nudging, version details/actions scroll below a fixed preview, and video versions can extract the current decoded frame as a new PNG image asset.
...
- **2026-05-22:** Exposed timeline track reordering through the track-header context menu. Video/audio tracks can now move up or down, preserving attached clips because clips remain keyed to the stable track ID; reordering also invalidates preview because video track order affects compositing.
- **2026-05-22:** Updated the New Generative Video modal so FPS, frame count, and seconds are all editable field-height controls. Seconds is derived from frames/FPS and editing seconds writes back to frame count, removing the old read-only duration label.
- **2026-05-22:** Enabled xAI Grok video as a first-class cloud video provider. The xAI video template creates a `grok-imagine-video` provider with prompt, duration, aspect ratio, and resolution inputs, shares the app-managed xAI credential, submits `/v1/videos/generations`, polls `/v1/videos/{request_id}`, downloads the temporary MP4, and stores it through the existing generated-version pipeline.
